...
async fn search_three_scopes() {
    ...
    Mock::given(method("GET"))
        ...
        .respond_with(ResponseTemplate::new(200).set_body_json(json!([{"id":1,"title":"bug A"}])))
        .mount(&server)
        .await;
    Mock::given(method("GET"))
        .and(path("/api/v4/groups/atoms/search"))
    ...
    Mock::given(method("GET"))
        ...
        .and(query_param("search", "fn"))
        .respond_with(ResponseTemplate::new(200).set_body_json(json!([{"filename":"main.rs"}])))
        .mount(&server)
        .await;

    let base = server.uri();
    env_cmd(&base)
        .args(["search", "--scope", "issues", "--query", "bug"])
        .assert()
        .success()
        .stdout(contains("bug A"));
    env_cmd(&base)
        .args([
            "search", "--scope", "commits", "--query", "x", "--group", "atoms",
        ])
        .assert()
        .success()
        .stdout(contains("abc"));
