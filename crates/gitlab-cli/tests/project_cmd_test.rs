...
async fn project_get_by_path() {
    ...
    Mock::given(method("GET"))
        ...
        .respond_with(
            ResponseTemplate::new(200)
                .set_body_json(json!({"id":9, "path_with_namespace":"group/proj"})),
        )
        .mount(&server)
        .await;
    env_cmd(&server.uri())
        .args(["project", "get", "group/proj"])
        .assert()
        .success()
        .stdout(contains("group/proj"));
}
