...
pub async fn run(ctx: Context, cmd: LabelCmd) -> Result<()> {
    match cmd {
        LabelCmd::List(a) => {
            let stream =
                PagedStream::<serde_json::Value>::start(&ctx.client, labels::list(&a.project));
        ...
        LabelCmd::Create(a) => {
            ...
            let v: serde_json::Value = ctx
                .client
                .send_json(labels::create(&a.project, &a.name, &a.color))
                .await?;
            emit_object(&v)?;
        }
        LabelCmd::Update(a) => {
            ...
            let v: serde_json::Value = ctx
                .client
                .send_json(labels::update(&a.project, a.id, &body))
                .await?;
            emit_object(&v)?;
        }
        LabelCmd::Delete(t) => {
            ...
            let _ = ctx
                .client
                .send_raw(labels::delete(&t.project, t.id))
                .await?;
        }
        LabelCmd::Subscribe(t) => {
            let v: serde_json::Value = ctx
                .client
                .send_json(labels::subscribe(&t.project, t.id))
                .await?;
            emit_object(&v)?;
        }
        LabelCmd::Unsubscribe(t) => {
            let v: serde_json::Value = ctx
                .client
                .send_json(labels::unsubscribe(&t.project, t.id))
                .await?;
            emit_object(&v)?;
        }
    }
