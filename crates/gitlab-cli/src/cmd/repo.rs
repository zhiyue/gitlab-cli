...
pub struct TreeArgs {
    ...
    #[arg(long = "ref")] pub rref: Option<String>,
...
pub struct ArchiveArgs {
    #[arg(long)] pub project: String,
    #[arg(long)] pub sha: Option<String>,
...
pub async fn run(ctx: Context, cmd: RepoCmd) -> Result<()> {
    match cmd {
        ...
        RepoCmd::Compare(a) => {
            let v: serde_json::Value = ctx.client.send_json(repos::compare(&a.project, &a.from, &a.to)).await?;
            emit_object(&v)?;
        }
        ...
        RepoCmd::MergeBase(a) => {
            let v: serde_json::Value = ctx.client.send_json(repos::merge_base(&a.project, &a.refs)).await?;
            emit_object(&v)?;
        }
    }
    Ok(())
