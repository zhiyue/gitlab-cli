...
pub struct WriteArgs {
    #[arg(long)] pub project: String,
    #[arg(long)] pub path: String,
    #[arg(long)] pub branch: String,
    #[arg(long)] pub content: String,
    #[arg(long)] pub message: String,
}
...
pub async fn run(ctx: Context, cmd: FileCmd) -> Result<()> {
    match cmd {
        FileCmd::Get(a) => {
            let v: serde_json::Value = ctx.client.send_json(files::get(&a.project, &a.path, &a.rref)).await?;
            emit_object(&v)?;
        }
        FileCmd::Raw(a) => {
        ...
        FileCmd::Blame(a) => {
            let v: serde_json::Value = ctx.client.send_json(files::blame(&a.project, &a.path, &a.rref)).await?;
            emit_object(&v)?;
        }
        FileCmd::Create(a) => {
            ...
            let v: serde_json::Value = ctx.client.send_json(files::create(&a.project, &a.path, &a.branch, &a.content, &a.message)).await?;
            emit_object(&v)?;
        }
        FileCmd::Update(a) => {
            ...
            let v: serde_json::Value = ctx.client.send_json(files::update(&a.project, &a.path, &a.branch, &a.content, &a.message)).await?;
            emit_object(&v)?;
        }
        FileCmd::Delete(a) => {
