...
fn masked_token_hides_middle() {
    assert_eq!(
        MaskedToken("glpat-ABCDEFGHIJKL").to_string(),
        "glpa****IJKL"
    );
...
async fn error_never_contains_literal_token() {
    ...
    assert!(
        !msg.contains("glpat-SHOULDNEVERAPPEAR"),
        "display leaked token: {msg}"
    );
    assert!(
        !dbg.contains("glpat-SHOULDNEVERAPPEAR"),
        "debug leaked token: {dbg}"
    );
    assert!(
        !payload.contains("glpat-SHOULDNEVERAPPEAR"),
        "payload leaked token: {payload}"
    );
}
