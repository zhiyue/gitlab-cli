...
pub fn list(project: &str, status: Option<&str>) -> PageRequest {
    ...
    if let Some(s) = status {
        p.query.push(("status".into(), s.into()));
    }
    p
}
...
pub fn get(project: &str, id: u64) -> RequestSpec {
    RequestSpec::new(
        Method::GET,
        format!("projects/{}/pipelines/{id}", encode_id(project)),
    )
}
...
pub fn create(project: &str, rref: &str) -> RequestSpec {
    RequestSpec::new(
        Method::POST,
        format!("projects/{}/pipeline", encode_id(project)),
    )
    .with_query([("ref", rref)])
}
...
pub fn retry(project: &str, id: u64) -> RequestSpec {
    RequestSpec::new(
        Method::POST,
        format!("projects/{}/pipelines/{id}/retry", encode_id(project)),
    )
}
...
pub fn cancel(project: &str, id: u64) -> RequestSpec {
    RequestSpec::new(
        Method::POST,
        format!("projects/{}/pipelines/{id}/cancel", encode_id(project)),
    )
}
...
pub fn variables(project: &str, id: u64) -> PageRequest {
    PageRequest::new(format!(
        "projects/{}/pipelines/{id}/variables",
        encode_id(project)
    ))
}
