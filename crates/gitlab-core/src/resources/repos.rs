...
pub fn tree(project: &str, path: Option<&str>, rref: Option<&str>, recursive: bool) -> PageRequest {
    ...
    if let Some(pp) = path {
        p.query.push(("path".into(), pp.into()));
    }
    if let Some(r) = rref {
        p.query.push(("ref".into(), r.into()));
    }
    if recursive {
        p.query.push(("recursive".into(), "true".into()));
    }
    p
}
...
pub fn compare(project: &str, from: &str, to: &str) -> RequestSpec {
    RequestSpec::new(
        Method::GET,
        format!("projects/{}/repository/compare", encode_id(project)),
    )
...
pub fn merge_base(project: &str, refs: &[String]) -> RequestSpec {
    let mut s = RequestSpec::new(
        Method::GET,
        format!("projects/{}/repository/merge_base", encode_id(project)),
    );
    for r in refs {
        s.query.push(("refs[]".into(), r.clone()));
    }
    s
}
